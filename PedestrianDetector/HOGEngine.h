...
namespace HOG
{
	...
	public:
		...
		void CalculateHOGDescriptorP(float *hostImage, int width, int height, int noOfHistogramBins,
                                     int windowSizeX, int windowSizeY, int cellSizeX, int cellSizeY,
                                     int blockSizeX, int blockSizeY, float *hostDesc, float *gradient);
		
		void InitHOGDescriptorCalculatorP(int width, int height, int noOfHistogramBins,
                                          int windowSizeX, int windowSizeY, int cellSizeX, int cellSizeY,
                                          int blockSizeX, int blockSizeY);

		void HOGDescriptorCalculatorP(float *hostImage, float *hostDesc);
		
		void HOGDescriptorCalculatorP(float *hostImage, float *hostDesc, float *gradient);
		
		void FreeHOGDescriptorCalculatorP();
		
		void GetImage(HOGImage *imageCUDA, ImageType imageType);
		void GetHOGDescriptorP(float *desc);
