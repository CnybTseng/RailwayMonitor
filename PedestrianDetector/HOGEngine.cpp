...
extern "C" void CalculateHOGDescriptor(float *hostImage, int width, int height, int noOfHistogramBins,
                                       int windowSizeX, int windowSizeY, int cellSizeX, int cellSizeY,
                                       int blockSizeX, int blockSizeY, float *hostDesc, float *gradient);

extern "C" void InitHOGDescriptorCalculator(int width, int height, int noOfHistogramBins,
                                            int windowSizeX, int windowSizeY, int cellSizeX, int cellSizeY,
                                            int blockSizeX, int blockSizeY, float wtscale);

extern "C" void HOGDescriptorCalculator(float *hostImage, float *hostDesc);

extern "C" void HOGDescriptorCalculatorOG(float *hostImage, float *hostDesc, float *gradient);

extern "C" void FreeHOGDescriptorCalculator();										   
...
extern "C" void GetHOGDescriptor(float *hostDesc);										   
...
void HOGEngine::InitializeHOG(int iw, int ih, float svmBias, float* svmWeights, int svmWeightsCount)
{
	...
	this->hBlockSizeX = 2;
	this->hBlockSizeY = 2;
	this->hWindowSizeX = 16;
	this->hWindowSizeY = 32;
	this->hNoOfHistogramBins = 9;
...
void HOGEngine::EndProcess()
{
	...
	if (formattedResults)
	{
		delete [] formattedResults;
		formattedResults = NULL;
	}
}

void HOGEngine::CalculateHOGDescriptorP(float *hostImage, int width, int height, int noOfHistogramBins,
                                        int windowSizeX, int windowSizeY, int cellSizeX, int cellSizeY,
                                        int blockSizeX, int blockSizeY, float *hostDesc, float *gradient)
{
	CalculateHOGDescriptor(hostImage, width, height, noOfHistogramBins, windowSizeX, windowSizeY, cellSizeX, cellSizeY, blockSizeX,
		blockSizeY, hostDesc, gradient);
}

void HOGEngine::InitHOGDescriptorCalculatorP(int width, int height, int noOfHistogramBins,
                                             int windowSizeX, int windowSizeY, int cellSizeX, int cellSizeY,
                                             int blockSizeX, int blockSizeY)
{
	InitHOGDescriptorCalculator(width, height, noOfHistogramBins, windowSizeX, windowSizeY, cellSizeX, cellSizeY, blockSizeX, blockSizeY, 2.0f);
}

void HOGEngine::HOGDescriptorCalculatorP(float *hostImage, float *hostDesc)
{
	HOGDescriptorCalculator(hostImage, hostDesc);
}

void HOGEngine::HOGDescriptorCalculatorP(float *hostImage, float *hostDesc, float *gradient)
{
	HOGDescriptorCalculatorOG(hostImage, hostDesc, gradient);
}

void HOGEngine::FreeHOGDescriptorCalculatorP()
{
	FreeHOGDescriptorCalculator();
}
...
void HOGEngine::GetHOGDescriptorP(float *desc)
{
	GetHOGDescriptor(desc);
}
...
void HOGEngine::NonmaximumSupression()
{
	int i, j, k;
	int leftoverX, leftoverY, currentWidth, currentHeight, rNumberOfWindowsX, rNumberOfWindowsY;
		...
		for (j=0; j<hNumberOfWindowsY; j++)
		{
			for (k=0; k<hNumberOfWindowsX; k++)
			{
				...
				if (score > 0)
				{
					...
					detect_results.push_back(DetectResult{hogResult.x, hogResult.y, hogResult.width, hogResult.height,
						hogResult.scale, hogResult.score});
				}
			}
		}
	...
	std::vector<DetectResult> merge_results;
	NonmaximumSuppression(detect_results, 0.8f, 32, 64, merge_results);
	
	nmsResultsCount = (int)merge_results.size();
	
	nmsResults = new HOGResult[hWindowSizeX * hWindowSizeX];
	for (size_t i = 0; i < merge_results.size(); i++) {
		nmsResults[i].x = merge_results[i].lx;
		nmsResults[i].y = merge_results[i].ty;
		nmsResults[i].width = merge_results[i].width;
		nmsResults[i].height = merge_results[i].height;
		nmsResults[i].scale = merge_results[i].scale;
		nmsResults[i].score = merge_results[i].score;
	}
}