...
__host__ void InitConvolution(int width, int height, bool useGrayscale)
{
	...
	checkCudaErrors( cudaMemcpyToSymbol(d_Kernel, h_Kernel, convKernelSize) );

	if (useGrayscale)
		checkCudaErrors(cudaMalloc((void**) &convBuffer1, sizeof(float1) * width * height));
	else
		checkCudaErrors(cudaMalloc((void**) &convBuffer4, sizeof(float4) * width * height));
}
