...
% url: http://zhiqiang.org/blog/it/matlab-data-structures.html
    ...
    methods
		...
		function flag = find(obj, el, fun, update)
			if obj.size()
                if obj.beg <= obj.rear 
                    for i = obj.beg : obj.rear-1
						flag = fun(obj.buffer{i}, el);
						if flag
							% disp('find same solution Tabu list.');
							% disp(obj.buffer{i});
							% disp(el);
							if update
								% disp('update');
                    ...
                    for i = 1 : obj.rear-1
                        flag = fun(obj.buffer{i}, el);
						if flag
							% disp('find same solution in Tabu list.');
							% disp(obj.buffer{i});
							% disp(el);
							if update
								% disp('update');
